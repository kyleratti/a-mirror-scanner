...
const downloaders_1 = __importDefault(require("../downloaders"));
    ...
    start() {
        ...
        lib_1.configurator.reddit.scanSubsList.forEach(subName => {
            ...
            stream.on('submission', function (post) {
                ...
                let id = post.id;
                let url = post.url;
                let agent = downloaders_1.default.match(url);
                if (!agent)
                    return console.error(`unable to find downloader agent that can handle '${url}'; aborting`);
